// Updated RCON Bridge with multiple slay command formats
    ...
    setupRoutes() {
        ...
        this.app.post('/rcon/slay', async (req, res) => {
            try {
                ...
                const targets = [];
                if (playerSteamId) targets.push(playerSteamId);
                if (playerName) targets.push(playerName);
                
                console.log(`🎯 Attempting slay with targets: ${targets.join(', ')}`);
                console.log(`📋 Reason: ${reason || 'No reason provided'}`);
                
                // Try multiple slay command formats
                const commandFormats = [
                    'Slay', 'AdminKill', 'Kill', 'AdminSlay'
                ];
                
                let lastResult = null;
                
                for (const target of targets) {
                    for (const cmdFormat of commandFormats) {
                        const slayCommand = `${cmdFormat} ${target}`;
                        console.log(`🧪 Trying: ${slayCommand}`);
                        ...
                        try {
                            ...
                            console.log(`❌ ${slayCommand} failed:`, error.message);
                        }
                    }
                }
                ...
                res.json({
                    success: true,
                    message: `Multiple slay commands attempted for player`,
                    targets: targets,
                    commandsAttempted: commandFormats.length * targets.length,
                    reason: reason,
